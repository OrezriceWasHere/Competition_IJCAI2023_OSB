...
from rl_trainer.algo.TD3 import TD3, ReplayBuffer
...
def main(args):
    ...
    # print(env.get_single_action_space(1)[0].high[0])
    # max_action = float(env.action_space.high[0])
    max_action = env.get_single_action_space(1)[0].high[0]

    if args.load_model:         #setup algos
        model = TD3(obs_dim, 2, [100, 30])
    ...
    replay_buffer = ReplayBuffer(obs_dim, 2, max_size=100000)
    ...
    while episode < args.max_episodes:
        ...
        while True:
            ...
            if step < random_steps:
                action_ctrl_raw = env.get_single_action_space(1)[0].sample()[0], env.get_single_action_space(1)[1].sample()[0]
            else:
                action_ctrl_raw = model.select_action(obs_ctrl_agent)
                # action = (
                #     model.select_action(obs_ctrl_agent)
                #     + np.random.normal(0, max_action * args.expl_noise, size=action_dim)
                # ).clip(-max_action, max_action)
            # print(action_ctrl_raw)
            # print(actions_map)
                            #inference
            # action_ctrl = actions_map[action_ctrl_raw]
            action_ctrl = [[action_ctrl_raw[0]], [action_ctrl_raw[1]]]        #wrapping up the action
            ...
            if args.train_model:
                replay_buffer.add(obs_ctrl_agent, action_ctrl_raw, next_obs_ctrl_agent, post_reward[ctrl_agent_index], done)
            ...
            if done:
                ...
                break
        if episode % args.save_interval == 0 and args.train_model:
